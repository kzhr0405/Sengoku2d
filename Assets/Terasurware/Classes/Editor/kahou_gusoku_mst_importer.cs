...
public class kahou_gusoku_mst_importer : AssetPostprocessor
{
    private static readonly string filePath = "Assets/Resources/Data/kahou_gusoku_mst.xls";
    private static readonly string[] sheetNames = { "kahou_gusoku_mst", };
        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
                    ...
                    var data = (Entity_kahou_gusoku_mst)AssetDatabase.LoadAssetAtPath(exportPath, typeof(Entity_kahou_gusoku_mst));
                    if (data == null)
                    {
                        data = ScriptableObject.CreateInstance<Entity_kahou_gusoku_mst>();
                    ...
                    for (int i=1; i<= sheet.LastRowNum; i++)
                    {
                        ...
                        var p = new Entity_kahou_gusoku_mst.Param();
