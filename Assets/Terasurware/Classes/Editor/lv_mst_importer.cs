...
public class lv_mst_importer : AssetPostprocessor
{
    private static readonly string filePath = "Assets/Resources/Data/lv_mst.xls";
    private static readonly string[] sheetNames = { "lv_mst", };
        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
                    ...
                    var data = (Entity_lv_mst)AssetDatabase.LoadAssetAtPath(exportPath, typeof(Entity_lv_mst));
                    if (data == null)
                    {
                        data = ScriptableObject.CreateInstance<Entity_lv_mst>();
