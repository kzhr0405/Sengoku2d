...
public class kahou_meiba_mst_importer : AssetPostprocessor
{
    private static readonly string filePath = "Assets/Resources/Data/kahou_meiba_mst.xls";
    private static readonly string[] sheetNames = { "kahou_meiba_mst", };
        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
                    ...
                    var data = (Entity_kahou_meiba_mst)AssetDatabase.LoadAssetAtPath(exportPath, typeof(Entity_kahou_meiba_mst));
                    if (data == null)
                    {
                        data = ScriptableObject.CreateInstance<Entity_kahou_meiba_mst>();
                    ...
                    for (int i=1; i<= sheet.LastRowNum; i++)
                    {
                        ...
                        var p = new Entity_kahou_meiba_mst.Param();
