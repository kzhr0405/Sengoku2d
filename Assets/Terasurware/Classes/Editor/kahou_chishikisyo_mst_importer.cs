...
public class kahou_chishikisyo_mst_importer : AssetPostprocessor
{
    private static readonly string filePath = "Assets/Resources/Data/kahou_chishikisyo_mst.xls";
    private static readonly string[] sheetNames = { "kahou_chishikisyo_mst", };
        ...
        foreach (string asset in importedAssets)
        {
            ...
            using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read))
            {
                ...
                foreach (string sheetName in sheetNames)
                {
                    ...
                    var data = (Entity_kahou_chishikisyo_mst)AssetDatabase.LoadAssetAtPath(exportPath, typeof(Entity_kahou_chishikisyo_mst));
                    if (data == null)
                    {
                        data = ScriptableObject.CreateInstance<Entity_kahou_chishikisyo_mst>();
                    ...
                    for (int i=1; i<= sheet.LastRowNum; i++)
                    {
                        ...
                        var p = new Entity_kahou_chishikisyo_mst.Param();
					...
					cell = row.GetCell(14); p.kahouNameChn = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(15); p.kahouExpChn = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(16); p.kahouTargetChn = (cell == null ? "" : cell.StringCellValue);
